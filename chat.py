...
# All user-facing responses must be Taiwan Traditional Chinese — qwen models
# default to Simplified, so enforce it explicitly on every response-generating call.
RESPONSE_ZH = "請一律用台灣正體中文（繁體字）回答，禁止出現任何簡體字。"
...
def handle_compilation(
    ...
    summary = llm_chat(summary_prompt, model=ARKIV_CHAT_MODEL, system=RESPONSE_ZH, conversation=history)
    return {
