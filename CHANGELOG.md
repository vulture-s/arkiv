...
### Added
- **`arkiv_cli.py status` + `queue` — operator CLI for the resource-aware pipeline (Phase 11.5e).** `python arkiv_cli.py status [--json]` prints the resource_probe snapshot (GPU/unified-mem %, loaded Ollama models), the active queue depth, and the backpressure decision (PROCEED/WAIT) the ingest pipeline would make right now. `python arkiv_cli.py queue list [--status S] [--json] | cancel <id> | retry <id>` gives visibility + control over the SQLite-backed ingest job queue (`jobs.py`). Pure glue over the already-shipped `resource_probe` + `jobs` modules; 8 tests in `tests/test_arkiv_cli.py`. (The remaining 11.5 item is the `OLLAMA_NUM_PARALLEL=2` A/B benchmark, which needs a GPU box.)
