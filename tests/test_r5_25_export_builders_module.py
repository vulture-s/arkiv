...
def test_http_and_log_helpers_did_not_go_to_export_builders():
    ...
    # been pulled into export_builders. In the R5-25 router split they moved to the
    # routers that own their callers: _attachment_headers → routers/export.py (with
    # the export routes), _log_safe → routers/misc.py (with /api/client-log).
    import export_builders
    assert not hasattr(export_builders, "_attachment_headers")
    assert not hasattr(export_builders, "_log_safe")
    import routers.export
    assert hasattr(routers.export, "_attachment_headers")
    import routers.misc
